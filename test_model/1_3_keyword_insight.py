...
NEWS_WITH_LABEL_PATH = "data/news_with_issue.csv"  # build_trend_ranking에서 저장하게 해도 됨
...
def get_keyword_insights(df: pd.DataFrame, year: int, month: int, keyword: str, top_n_keywords: int = 30, top_n_articles: int = 5):
    ...
    texts = (sub["title"].fillna("") + " " + sub["body"].fillna("")).tolist()
    ...
    return keywords, sub[["date", "title", "body", "issue_confidence"]]
